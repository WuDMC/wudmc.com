...
export default function WaveTerrainCustomizable() {
  ...
  useEffect(() => {
    ...
    // Calculate terrain size dynamically based on screen size to ensure edges are never visible
    const aspectRatio = width / height;
    const dynamicTerrainWidth = Math.max(60, width / 15); // Scale with screen width
    const dynamicTerrainDepth = Math.max(40, dynamicTerrainWidth / aspectRatio);
    ...
    // Create terrain with dynamic size
    ...
    // Create contour lines with dynamic size
    const lines = [];
    for (let i = 0; i < params.lineCount; i++) {
      ...
      for (let j = 0; j <= segments; j++) {
        points.push(new THREE.Vector3(
          (j / segments) * dynamicTerrainWidth - dynamicTerrainWidth / 2,
          0,
          (i / params.lineCount) * dynamicTerrainDepth - dynamicTerrainDepth / 2
        ));
      }
    ...
    sceneRef.current = {
      ...
      lines,
      terrainMaterial,
      terrainWidth: dynamicTerrainWidth,
      terrainDepth: dynamicTerrainDepth,
    };
    ...
    const animate = () => {
      ...
      for (let i = 0; i < positions.length; i += 3) {
        ...
        if (p.waveType === 'surf') {
          ...
          const depthFactor = 1 + (z + sceneRef.current.terrainDepth / 2) / sceneRef.current.terrainDepth * 0.5;
        ...
        if (p.mouseInfluence > 0) {
          const mx = mousePos.current.x * sceneRef.current.terrainWidth / 2;
          const mz = mousePos.current.y * sceneRef.current.terrainDepth / 2;
      ...
      lines.forEach((line, lineIndex) => {
        ...
        const z = (lineIndex / lines.length) * sceneRef.current.terrainDepth - sceneRef.current.terrainDepth / 2;
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      ...
      {/* Secret hint */}
      {showHint && (
        <div className="absolute bottom-4 right-4 z-20 bg-black/60 text-white px-3 py-2 rounded-lg text-xs animate-fade-in">
          Press <kbd className="px-1.5 py-0.5 bg-white/20 rounded">W</kbd> or double-click
        </div>
      )}
